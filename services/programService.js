...
var invitedUserSchema = require('../models/invited_user');
...
var ProgramService = {
    programAddEdit: (req_data, callback) => {
        ...
        if(!req_data.program_name){
            callback({ success: false, error: true, message: "Program name require" });
        }else if(!req_data.program_datetime){
            callback({ success: false, error: true, message: "Program date and time require" });
            ...
            async.waterfall([
                ...
                function (nextcb) {
                    var search_query = {};
                    var date_time = moment(new Date(req_data.program_datetime)).format('YYYY-MM-DD HH:mm:ss');
                    if(req_data.program_id){ 
                        search_query = { $or: [ { _id: req_data.program_id}, { program_name : { $regex: '^'+req_data.program_name+'$', $options: "$i" }, datetime : date_time  }] };    
                    }else{
                        search_query = { program_name : { $regex: '^'+req_data.program_name+'$', $options: "$i" }, datetime : date_time };
                    }
                    Program.find(search_query).exec(function(matcherr, similar_program){
                        if (matcherr) {
                            nextcb({ success: false, error: true, status: STATUS_CONSTANTS.INTERNAl_DB_ERROR, message: STATUS_MESSAGES.INTERNAl_DB_ERROR, errors: matcherr });
                        } else if(!req_data.program_id && similar_program.length > 0){
                            nextcb({ success: false, error: true, message: "Program title and time matched with other program" });
                        } else if(req_data.program_id && similar_program.length == 0){
                            nextcb({ success: false, error: true, message: "Program id not found" });
                        } else if(req_data.program_id && ( similar_program.length > 1 || req_data.program_id != similar_program[0]._id)){
                            nextcb({ success: false, error: true, message: "Program title and time matched with other program" });
                        } else{
                            nextcb(null);
                        }
    ...
    addInvitedUser: async (data,callback)=>{
        ...
        } else 
        {
            ...
            new invitedUserSchema(data).save(function (err, result) 
            {
                if (err) {
                    callback({
                        "response_code": 5005,
                    ...
                    callback({
                        "response_code": 2000,
                        "response_message": "Invitation Added  successfully."
                    });
                }
            });
        }
    },
    checkInviteStatus: async (data,callback)=>{
        ...
        {
            let query ={};
            query['phone_no']= new RegExp(data.phone_no,'i');
            ...
            //       ]
            // };
            // query['activity']= new RegExp(data.activity,'i');
            // if(data.name){
            ...
            let invitedUser = await invitedUserSchema.find(query,project, function (err, result) {
                if (err) {
                    callback({
                        "response_code": 5005,
                    ...
                    callback({
                        "response_code": 2000,
                        "response_message": 'User invitation status',
